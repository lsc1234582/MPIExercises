               ...
               MLIFETiming *timedata, void *privateData )
{
     ...
     * ------------------------------------ -
     */
    MPI_Datatype EdgeElemType;
    MPI_Type_create_resized(MPI_INT, 0, LCols + 2, &EdgeElemType);
    MPI_Type_commit(&EdgeElemType);

    /* first, move the left, right edges */
    MPI_Isend(&(matrix[1][1]), LRows, EdgeElemType, patch->left, 0, comm, reqs);
    MPI_Irecv(&(matrix[1][0]), LRows, EdgeElemType, patch->left, 0, comm, reqs+1);
    MPI_Isend(&(matrix[1][LCols]), LRows, EdgeElemType, patch->right, 0, comm, reqs+2);
    MPI_Irecv(&(matrix[1][LCols + 1]), LRows, EdgeElemType, patch->right, 0, comm, reqs+3);
