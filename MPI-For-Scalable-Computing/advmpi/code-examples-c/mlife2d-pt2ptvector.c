               ...
               MLIFETiming *timedata, void *privateData )
{
    ...
    /* Intialise a vector type for sending (non-contiguous) left and right edges
     * Initialise a vector type for receiving (non-contiguous) left and right edges
     *        |<- lnj            ->|      
     * ------------------------------------ -
     * || lr1 | ls1 | eo ... | rs1 | rr1 || 
     * || lr2 | ls2 |  ...   | rs2 | rr2 || lni
    ...
    MPI_Isend(&(matrix[1][1]), 1, MPI_INT, patch->left, 0, comm, reqs);
    MPI_Irecv(&(matrix[1][0]), 1, VertEdgeType, patch->left, 0, comm, reqs+1);
    MPI_Isend(&(matrix[1][LCols]), 1, MPI_INT, patch->right, 0, comm, reqs+2);
